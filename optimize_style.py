...
KO_PHONETIC_BASS_TEXTS = [
    ...
    # 2. 발음 교정용: 파찰음(ㅈ, ㅊ) 및 마찰음(ㅅ, ㅆ) 연속 구간 (조음점 명확화)
    '차갑게 식어버린 철창 너머로, 짙은 잿빛 구름이 잔잔하게 흩어지고 있었다. 진실은 늘 찰나의 순간에 자취를 감춘다.',
    # 3. 발음 교정용: 겹받침 및 연음 법칙 강제 유도 (자연스러운 발화 리듬 학습)
    '맑고 쾌청한 가을 하늘 아래, 붉은 단풍잎이 흙바닥 위로 넓게 깔려 있었다. 삶은 닭고기를 썰어 넣은 국물은 끓일수록 깊은 맛을 냈다.',
    # 4. 발음 교정용: 파열음(ㄱ, ㄷ, ㅂ) 격음/경음 대비 (타격감 있는 저음 발성)
    '단단한 바위가 부서지는 굉음과 함께, 까만 밤하늘로 불꽃이 튀어 올랐다. 뚜렷한 목표가 없다면 텅 빈 껍데기에 불과하다.'
]
